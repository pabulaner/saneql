...
struct hash<Varchar<maxLen>> {
    std::size_t operator()(const Varchar<maxLen>& v) const {
...
int main(int argc, char** argv) {
    ...
    while (run) {
        ...
        if (params[0] == "help") {
            ...
            std::cout << "          run [--file opt:name] [--repeat number] [--limit number] [--disableperf] = runs the provided files (queries)" << std::endl;
            ...
            for (size_t i = 1; i < params.size(); i++) {
                ...
                if (params[i] == "--file" && !param) {
                    ...
                    limitParam = true;
                } else if (params[i] == "--disableperf" && !param) {
                    disablePerfValue = true;
                } else if (fileParam) {
                    fileParam = false;
