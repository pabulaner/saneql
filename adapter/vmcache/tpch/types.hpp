...
// namespace std {
// template <typename T>
// requires requires { tpch::type_tag<T>::TAG; }
// struct hash<T> {
//   std::size_t operator()(const T &k) const {
//     using _hasher = ::tpch::HashKey<T>;
...
//     using _hasher = ::tpch::HashKey<std::tuple<Args...>>;
//     static constexpr _hasher h = _hasher();
//     return h(k);
//   }
// };
...
// --------------------------------------------------------------------------
// template <>
// struct fmt::formatter<tpch::Date> {
//   // Parses format specifications; we have none at the moment
//   constexpr auto parse(format_parse_context& ctx) -> decltype(ctx.begin()) {
//       return ctx.end();
//   }
//   template <typename FormatContext>
//   auto format(const tpch::Date& d, FormatContext& ctx) const -> decltype(ctx.out()) {
//       // ctx.out() is an output iterator to write to.
//       unsigned year, month, day;
//       tpch::Date::fromInt(d.value, year, month, day);
